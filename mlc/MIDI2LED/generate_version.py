...
#define DISPLAY_VERSION_STRING "{display_version_string}"
...
def main(argv):
    ...
    match = re.match(
        r'v([0-9]+)\.([0-9]+)(\.([0-9]+))*-([0-9]+)-g([a-fA-F0-9]+)',
        git_describe,
    )
    ...
    commits_past_tag = match.group(5)
    commit_id = match.group(6)
    ...
    digits_used = sum(c.isdigit() for c in version_number)
    MAX_DIGITS = 4
    if digits_used > MAX_DIGITS:
        raise ValueError(f"version number {version_number} too long to display on device")

    prefix = 'v' if digits_used < MAX_DIGITS else ''
    pad = ' ' * (MAX_DIGITS - digits_used - 1)
    display_version_string = f'{prefix}{pad}{version_number}'
